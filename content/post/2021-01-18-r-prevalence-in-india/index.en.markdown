...
keywords:
  - tech
summary: "How does R fare vis-a-vis Python in India? We explore this question using data from Kaggle MLDSS Survery 2020. We also analyze data from surveys in 2018 and 2019, to explore in trends in respondents from India."
---
...
1. We first contextualize the Data Science practice in India within the global scenario, highlighting R and Python prevalence.
...
A companion [Shiny app](https://manohar.shinyapps.io/kaggle_mldss/) enables a similar exploration for Top 10 countries (in terms of number of respondents), by reproducing all the plots in this blog post.
...
Even though R and Python are considered the top two preferred programming languages for data science and machine learning practitioners, the survey respondents use other languages too. While one would expect to R and Python as the top 2 languages of the respondents, the reality is quite different.
...
The plots show a gradual rise in the number of respondents who program in Python. As for R, the the number of respondents dipped from 4% in 2018 to 2% in 2019 and stayed at that level in 2020. There is an overall reduction in those that program in R, since the percentage of respondents who program in both R and Python has also gone down.
...
We will plot the actual numbers of respondents.
...
The most common titles reported by **R only** Programmers are Data Scientist, Data Analyst and Business Analyst: we also see dip in the number of respondents for these titles. In contrast, we notice a small uptick in the numbers of Statisticians, but this rise in Statistician numbers is not enough to cover the dip in Business Analysts and Data Analysts.
...
We first look at data from 2018 since there was a sizeable response in that year.

<img src="{{< blogdown/postref >}}index.en_files/figure-html/unnamed-chunk-9-1.png" width="672" />

Clearly, the tasks performed by Business Analysts and Data Analysts are similar. In fact, most of them "Analyze and understand data".
...
The following chart shows the percentage of respondents in different compensation levels. Respondents using R are represented well in all compensation levels.

For example, for 8% of all respondents who use R, the compensation is between $7,500 - $9,999, There are very few respondents at the very high compensation levels and there is R representation in those as well.
...
As opposed to R, Python programmers have a larger range of job titles. This perhaps explains to some extent the overall higher numbers for Python. However, the number of Python respondents are larger that those of R, even in these job titles that have R representation.
...
If the lower number of R respondents are representative of the overall numbers of R programmers in India, it is not clear why that would be.
...
Considering those that program in R only (the top row), the small numbers for `ggplot` do come as a surprise. The numbers for `Shiny` are also small and that could be because not everyone will want to share their results on the Web.
...
The plot below shows the different Machine Learning libraries used by R Programmers. On the left are ML related libraries most used by those respondents who program in R.
...
Let's explore usage of these ML Libraries across job titles. As with the visualization libraries, it appears that those who program in both R and Python use Caret and Tidymodels more often than those who program in R only.

Perhaps these who program in both R and Python are the true users of R.
...
In other words, to understand the level of R prevalence in India, we need to look at data science practitioners who use both R and Python, rather than those who just program in R. That makes sense too: both languages have their strong points and the choice of the language should depend on that task at end.

However, there is room for improving the use of R in India. Perhaps these need to be re-introduced to the community in India.

*Postscript*

This blog post was created using RMarkdown and rendered using the R package `blogdown`. All the analysis and plots were created using the [`tidyverse`](https://www.tidyverse.org) packages. The RMarkdown document can be found on [GitHub](https://github.com/rnpythonblog/rnpython/blob/main/content/post/2021-01-18-r-prevalence-in-india/index.en.Rmarkdown).
