...
A companion [Shiny app](https://manohar.shinyapps.io/kaggle_mldss/) reproduces all the plots in this blog post, to enable a similar analysis for Top10 countries with most number of respondents.

## Part I: India in the Global Context
The two charts below illustrate participation in the survey by data science and machine learning professions residing in India.
...
We compare the percentage of respondents for R and Python and as we can see from the chart on the right, Python percentages vary within a narrow range of 77% - 83%, but the R proportions vary in a wide range: 8% - 32%. As far as R is concerned, India is somewhere in the middle of this range, while USA tops the list in terms of percentage of its respondents who use R.
...
Even though R and Python are considered the top programming languages for data science and machine learning practitioners, the survey respondents come from a wide variety of backgrounds and use multiple languages. While one would expect to see R and Python as the top 2 languages of the respondents, the reality is quite different.

As can be seen from the bar graph on the left below, globally Python is the top choice and R is at the 3rd spot (not quite at #2, as we expect it to be.) The results for India are even more surprising: R drops to the 6th spot.
...
Overall, the percentage of respondents programming in R is less compared to Python even globally. But the numbers in India are even smaller. R clearly lags Python when it comes to respondents from India. Since the respondents do not form a randomized sample, we cannot say for sure that R adoption lags behind that of Python. But the difference between the two is too large to not consider the possibility that R adoption lags that of Python by a significant amount.

This sets the stage for Part 2. We will try and <font color = 'red'>**explore**</font> the reasons for the large gap between Python and R, in terms of respondents.

## Part II: R and Python over the years in India
We begin by looking at trends over the last three years in terms of percentage of Python vs R respondents.


<img src="{{< blogdown/postref >}}index.en_files/figure-html/unnamed-chunk-7-1.png" width="1344" />

From the plots above, we see a big growth for Python from 2019 to 2020, while R dips from 2018 to 2019 and dips a wee bit further in 2020.

 >In addition to the Python vs R comparison, we will explore the reasons dip in R respondents through the years.

### Exploring the dip: changes in respondent profile over the years
To investigate what has caused the dip in respondents who program in R, let's explore Educational Background and Job Titles reported by respondents to see if there is a change in the respondent profile over the years.
...
<img src="{{< blogdown/postref >}}index.en_files/figure-html/unnamed-chunk-11-1.png" width="1344" />
...
The following chart shows the percentage of respondents in each compensation level As we can see, R programmers are represented well in all compensation levels.

<img src="{{< blogdown/postref >}}index.en_files/figure-html/unnamed-chunk-12-1.png" width="1152" />

As we can see from the plot above, R programmers are well represented across most of the compensation categories. There are very few programmers at the very high compensation categories and there is R representation in those categories as well.
...
There is a large number of tools in the R eco-system for these tasks, not to mention that it is generally easy to start with R. The packages in the **`tidyverse`** universe make it relative easy to perform data analysis, **`Shiny`** makes it easy to share analysis on the web and there is a strong community support. R also has a large number of packages for Machine Learning. **<font color='red'>Additional surveys regarding attitude towards R may help shed more light on this</font>**.

We will now deep dive into the use of R, to better understand how R is being used in India.

## Part III: R Usage in India

In our analysis, while our focus is on respondents that program in R only, we include those respondents who program in both Python and R to see if there are any similarities.

### Activities Performed by R Programmers
We begin by looking at typical activities performed by R Programmers.

<img src="{{< blogdown/postref >}}index.en_files/figure-html/unnamed-chunk-13-1.png" width="1152" />
...
<font color='red'>Let's look at the number of activities performed by R programmers, on average</font>.

<img src="{{< blogdown/postref >}}index.en_files/figure-html/unnamed-chunk-14-1.png" width="1344" />

There is a wide variation in general, those in smaller teams perform fewer different types of tasks. Statisticians and Data Scientists seem to be performing a large variety of tasks, on average.

### Visualization Tools
Effective visualizations are important for data science teams.

The following plot shows the Visualization tools used by R programmers: we are primarily interested in seeing the use of **`ggplot`** and **`Shiny`**.

<img src="{{< blogdown/postref >}}index.en_files/figure-html/unnamed-chunk-15-1.png" width="1344" />

The findings are as expected. Those that program in Python and R use more of **`Matplotlib`** and **`Seaborn`**. What is interesting is that a comparative number of those who program in both Python and R, also use **`ggplot`**.
...
<font color='red'>Let's see if there is a differences in use of **`ggplot`** and **`Shiny`** across titles for **R Programmers**</font>

<img src="{{< blogdown/postref >}}index.en_files/figure-html/unnamed-chunk-16-1.png" width="1152" />

Considering those that program in R only, the small numbers for ggplot do come as a surprise. The numbers for Shiny are also small and that could be because not everyone will want to share their results on the Web.

However, if we look at those that program in both R and Python, we notice that Shiny is more popular with Data Analyst and Data Scientist. This is true of ggplot as well, but to a lesser extent. In addition, Business Analysts also use ggplot.

Overall, those that program in both Python and R seem to be leveraging the R eco-system better than those that program in R only.

### Machine Learning Libraries and Algorithms

The plot below shows the difference Machine Learning libraries used by R Programmers.

<img src="{{< blogdown/postref >}}index.en_files/figure-html/unnamed-chunk-17-1.png" width="1344" />

Clearly, R Programmers employ a large number of popular Machine Learning libraries. It is no surprise to see **Caret** top the list. It will be interesting to watch for **tidymodels** next year.
...
<img src="{{< blogdown/postref >}}index.en_files/figure-html/unnamed-chunk-18-1.png" width="1344" />

Linear Regression seems to be the most popular machine learning algorithm used. Indeed, as per the Executive Summary, Linear Regression is the most popular method among Data Scientists, so this does not come as a surprise.

### Size of Company and Size of Data Science Teams

We close this section with an exploration of the size of companies where R Programmers are employed. We also investigate typical team sizes.

We consider respondents that program in R only.

<img src="{{< blogdown/postref >}}index.en_files/figure-html/unnamed-chunk-19-1.png" width="1152" />
...
The large prevalence of Python perhaps attracts new comers to Python fueling a cycle: however, as the experience of my two friends suggests, this may not be desirable.
